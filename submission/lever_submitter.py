...
def dry_run_lever_submission(job, resume_path, cover_letter_path=None, candidate=None):
...
def submit_lever_application(job, resume_path, cover_letter_path=None, candidate=None, dry_run=True):
    if not dry_run:
        raise NotImplementedError("Live Lever submission is intentionally disabled for now.")

    return dry_run_lever_submission(
        job=job,
